-- EMERGENCY: Recreate the referral trigger that was not properly created

-- 1. Drop any existing trigger/function (clean slate)
DROP TRIGGER IF EXISTS on_user_referral ON users CASCADE;
DROP FUNCTION IF EXISTS process_referral() CASCADE;

-- 2. Create the referral processing function
