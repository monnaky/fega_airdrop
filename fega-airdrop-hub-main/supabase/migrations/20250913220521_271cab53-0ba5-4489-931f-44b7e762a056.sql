-- STEP 1: DROP ALL FUNCTION VARIANTS EXPLICITLY
DO $$
DECLARE
    r RECORD;
BEGIN
    -- Drop all versions of process_referral function
    FOR r IN (SELECT oid, proname FROM pg_proc WHERE proname = 'process_referral')
    LOOP
        EXECUTE 'DROP FUNCTION ' || r.oid::regprocedure || ' CASCADE';
    END LOOP;
    
    -- Drop all versions of admin_update_settings function
    FOR r IN (SELECT oid, proname FROM pg_proc WHERE proname = 'admin_update_settings')
    LOOP
        EXECUTE 'DROP FUNCTION ' || r.oid::regprocedure || ' CASCADE';
    END LOOP;
END
...
-- STEP 7: Create the clean referral trigger function
...
BEGIN
  ...
  VALUES ('🔥 TRIGGER: User ' || NEW.wallet_address || ' | referrer_id: ' || COALESCE(NEW.referrer_id::text, 'NULL'));
  ...
  IF NEW.referrer_id IS NOT NULL THEN
    ...
    INSERT INTO public.debug_log (message)
    VALUES ('✅ SUCCESS: Added ' || v_bonus || ' tokens to referrer ' || NEW.referrer_id::text);
  END IF;
...
-- STEP 9: Test with clean users
DO $$
DECLARE
  referrer_id UUID;
BEGIN
  INSERT INTO public.users (wallet_address) VALUES ('0xAAA') RETURNING id INTO referrer_id;
  INSERT INTO public.users (wallet_address, referrer_id) VALUES ('0xBBB', referrer_id);
END $$;